...
def u16(b, off): return (b[off]<<8) | b[off+1]
...
def l4_checksum_ok(pkt):
    ...
    if not ip or not (tcp or udp): return None
    if tcp:
        seg = bytes(tcp); proto = 6
    else:
        seg = bytes(udp); proto = 17
    if IP in pkt:
    ...
    if proto == 6: seg = seg[:16] + b'\x00\x00' + seg[18:]
    else:          seg = seg[:6]  + b'\x00\x00' + seg[8:]
...
def parse_tcp_options(tcp):
    ...
    for k,v in (tcp.options or []):
        if k == 'MSS':       opts.append(("opt_mss", v))
        elif k == 'WScale':  opts.append(("opt_wscale", v))
        elif k == 'SAckOK':  opts.append(("opt_sackok", 1))
        elif k == 'Timestamp': opts.append(("opt_tsval", v[0])); opts.append(("opt_tsecr", v[1]))
    return dict(opts)
...
def parse_tls_sni(payload_bytes):
    ...
    try:
        ...
        if len(hs) < 4+34: return None
        off = 4+34
        if off >= len(hs): return None
        sid_len = hs[off]; off += 1 + sid_len
        if off+2 > len(hs): return None
        cs_len = u16(hs, off); off += 2 + cs_len
        if off+1 > len(hs): return None
        cm_len = hs[off]; off += 1 + cm_len
        if off+2 > len(hs): return None
        ext_len = u16(hs, off); off += 2
        end = min(len(hs), off + ext_len)
        while off + 4 <= end:
            etype = u16(hs, off); elen = u16(hs, off+2); off += 4
            if etype == 0 and off + 2 <= end:
                lst_len = u16(hs, off); p = off+2
                while p + 3 <= off+2+lst_len and p + 3 <= end:
                    nt = hs[p]; nlen = u16(hs, p+1); p += 3
...
def five_tuple(pkt):
    if IP in pkt:
        s,d = pkt[IP].src, pkt[IP].dst; proto = pkt[IP].proto
    elif IPv6 in pkt:
        s,d = pkt[IPv6].src, pkt[IPv6].dst; proto = pkt[IPv6].nh
    else:
        return None
...
def flow_stats_for(pkt):
    key = five_tuple(pkt)
    if not key: return (0,0,0.0,0.0,0.0)
    f = flows[key]
    pkts = f["pkts"]; by = f["bytes"]
    if pkts and pkts > 1:
...
def feature_row(n, pkt, raw):
    ...
    t_rel = ts - (t0 or ts)
    # L2
    eth = safe_get(pkt, Ether); d11 = safe_get(pkt, Dot11)
    vlan = safe_get(pkt, Dot1Q)
    eth_type = None; vlan_id=None; vlan_prio=None
    if eth:
    ...
    # L3
    ip_ver=None; ip_src=None; ip_dst=None; ttl_hlim=None; dscp=None; ecn=None
    ip_df=None; ip_mf=None; ip_frag_off=None; ip4_ok=None
    if IP in pkt:
        ip = pkt[IP]; ip_ver=4; ip_src=ip.src; ip_dst=ip.dst
        ttl_hlim = ip.ttl; dscp=(ip.tos>>2)&0x3F; ecn=ip.tos&0x3
        ip_df = int(getattr(ip.flags, "DF", 0)); ip_mf = int(getattr(ip.flags, "MF", 0)); ip_frag_off = int(ip.frag)
        try: ip4_ok = int(bool(ipv4_hdr_checksum_ok(ip)))
        except Exception: ip4_ok = None
    elif IPv6 in pkt:
        ip = pkt[IPv6]; ip_ver=6; ip_src=ip.src; ip_dst=ip.dst
        ttl_hlim=ip.hlim; dscp=(getattr(ip,"tc",0)>>2)&0x3F; ecn=getattr(ip,"tc",0)&0x3
        ip_df=ip_mf=ip_frag_off=None; ip4_ok=None
    # L4
    l4_proto=None; sport=None; dport=None; tcp_flags=None; tcp_win=None; tcp_hlen=None; udp_len=None; l4_ok=None
    if TCP in pkt:
        t = pkt[TCP]; l4_proto=6; sport=t.sport; dport=t.dport
        tcp_flags=str(t.flags); tcp_win=int(t.window); tcp_hlen=(t.dataofs or 0)*4
    if UDP in pkt:
        u = pkt[UDP]; l4_proto=17; sport=u.sport; dport=u.dport; udp_len=int(u.len)
    try:
        ...
        if d.qr==1 and d.an:
            ...
            for i in range(int(d.ancount or 0)):
                ...
                if isinstance(rr, DNSRR):
                    if rr.type == 1 and dns_a is None: dns_a = str(rr.rdata)
                    if rr.type == 28 and dns_aaaa is None: dns_aaaa = str(rr.rdata)
                    if rr.type == 5 and dns_cname is None:
                        try: dns_cname = rr.rdata.decode(errors='ignore')
                        except: dns_cname = str(rr.rdata)
    tls_sni=None; http_method=None; http_host=None; http_path=None
    payload = bytes(pkt[Raw].load) if Raw in pkt else b""
    if TCP in pkt and payload:
        sni = parse_tls_sni(payload)
        if sni: tls_sni = sni
        m,h,p = parse_http(payload)
        if m: http_method=m; http_host=h; http_path=p
    # flow stats (after update_flow_stats was called)
    f_pkts, f_bytes, f_iat_min, f_iat_avg, f_iat_max = flow_stats_for(pkt)
    return {
        "frame_no": n, "ts_epoch": ts, "t_rel": t_rel, "len_bytes": len(raw),
        "eth_src": eth.src if eth else None, "eth_dst": eth.dst if eth else None, "eth_type": eth_type,
        "vlan_id": vlan_id, "vlan_prio": vlan_prio,
        "ip_version": ip_ver, "ip_src": ip_src, "ip_dst": ip_dst, "ttl_hlim": ttl_hlim, "dscp": dscp, "ecn": ecn,
        "ip_flags_df": ip_df, "ip_flags_mf": ip_mf, "ip_frag_off": ip_frag_off, "ipv4_checksum_ok": ip4_ok,
        "l4_proto": l4_proto, "sport": sport, "dport": dport, "tcp_flags": tcp_flags,
        "tcp_win": tcp_win, "tcp_hdr_len": tcp_hlen, "udp_len": udp_len, "l4_checksum_ok": l4_ok,
        "opt_mss": opt_mss, "opt_wscale": opt_wscale, "opt_sackok": opt_sackok, "opt_tsval": opt_tsval, "opt_tsecr": opt_tsecr,
        "dns_qname": dns_qname, "dns_a": dns_a, "dns_aaaa": dns_aaaa, "dns_cname": dns_cname,
        "tls_sni": tls_sni, "http_method": http_method, "http_host": http_host, "http_path": http_path,
        "flow_pkts": f_pkts, "flow_bytes": f_bytes, "flow_iat_min": f_iat_min, "flow_iat_avg": f_iat_avg, "flow_iat_max": f_iat_max
    }
...
def format_headers(p, n):
    ...
    rel = ts - (t0 or ts)
    lt = time.localtime(ts); ms = int((ts - int(ts)) * 1000)
    ts_human = time.strftime("%Y-%m-%d %H:%M:%S", lt) + f".{ms:03d}"
    lines = [f"\n=== Frame {n} ===", f"ts_epoch={ts:.6f} ts_local={ts_human} t_rel={rel:.6f}s"]
    eth = safe_get(p, Ether); vlan = safe_get(p, Dot1Q)
    if eth: lines.append(f"L2: Ethernet src={eth.src} dst={eth.dst} type=0x{eth.type:04x}")
    while vlan:
        lines.append(f"  VLAN id={vlan.vlan} pri={vlan.prio}")
        vlan = vlan.payload if isinstance(vlan.payload, Dot1Q) else None
    ip4 = safe_get(p, IP); ip6 = safe_get(p, IPv6)
    if ip4:
        dscp = (ip4.tos>>2)&0x3F; ecn = ip4.tos&0x3
        ...
        try: lines.append(f"  IPv4_checksum={'OK' if ipv4_hdr_checksum_ok(ip4) else 'BAD'}")
        except: lines.append("  IPv4_checksum=NA")
    if ip6:
        lines.append(f"L3: IPv6 {ip6.src}->{ip6.dst} nh={ip6.nh} hlim={ip6.hlim} tc={getattr(ip6,'tc',0)}")
    tcp = safe_get(p, TCP); udp = safe_get(p, UDP)
    if tcp:
    ...
    if (ip4 or ip6) and (tcp or udp):
        ...
        except: lines.append("  L4_checksum=NA")
    payload = bytes(p[Raw].load) if Raw in p else b""
    if tcp and payload:
        sni = parse_tls_sni(payload); 
        if sni: lines.append(f"TLS SNI={sni}")
        m,h,pp = parse_http(payload)
        if m: lines.append(f"HTTP {m} host={h} path={pp}")
    return "\n".join(lines)
...
def main():
    ...
    ap.add_argument("-f","--bpf",default=None)
    ...
    if args.bpf: print(f"Filter:       {args.bpf}")
